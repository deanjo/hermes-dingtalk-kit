...
已接收的用户原文、成功送达的卡片和 webhook（回调地址）回复保存在 `DINGTALK_KIT_STATE_DIR`，默认使用 `$HERMES_HOME/dingtalk-kit/dingtalk_card_replies.db`。沿用既有 SQLite 表结构，正文不按时间或条数淘汰；重启、升级、新开对话继续使用持久挂载的数据。`max_rows` 参数仅保留调用兼容性，不再限制原文条数。写入失败会记录日志，发送结果中的 `reply_context_saved=false` 不能当作已保存。

引用优先读取同一聊天内按消息编号保存的全文；钉钉提供的正文作为补充，按发送时间找到的内容明确标注为候选。所需当时对话从核心已保存的同聊天历史读取，截止到引用时刻，跨新会话也可恢复。未知或从未保存的内容不猜测回填，由模型解释缺口并追问。普通聊天、确认、否认、修改和追问保留当前原话交给模型；只有 `/help` 等显式命令允许程序控制，不再生成或消费引用确认码。

超过核心 500 字引用预览的原文会完整附在模型输入中。完整资料超过 120000 UTF-8 字节时，写入同一持久目录的 `quote-<sha256>.txt`，本轮请求和文件位置交给模型，要求按需分段读取；不能把没有读取的部分当作已理解。该值是输入组织阈值，不是拒绝对话或截断保存的上限。图片、文件仍沿用既有下载及媒体路径；历史附件失效时无法凭空恢复。
